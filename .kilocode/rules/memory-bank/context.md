...
| February 2026 | **Medicine Display Format & Hindi Language**: Updated prescription print format - Line 1: Medicine Name Potency (Quantity), Line 2: "Dose & Timing:" + natural language with separator lines between medicines. Added Hindi language support for dose timing and next visit text (medicine names stay in English). Language selector in print settings. |
| February 2026 | **Billing & Prescription Fixes**: Fixed free follow-up (0 fee) completing properly with auto-receipt generation. Added dose form after quantity in prescription display (e.g., "2dr Pills"). Added chief complaint toggle in print settings. Fixed receipt print button opening prescription instead of fee receipt. Fixed "Free Follow Up" vs "Exempt" status display throughout billing module. All status badges now correctly distinguish between free follow-up and exempt fees. |
| February 2026 | **Print Button Fixes**: Fixed all print buttons to print correct documents - prescription print buttons now print prescriptions (not receipts), fee receipt buttons print fee receipts, and bill buttons print bills. Created separate functions: handlePrintPrescription, handleWhatsAppPrescription for prescription view, and handlePrintReceipt, handleWhatsAppReceipt for fee receipts. Fixed receipt popup badge to show correct status (Free Follow Up vs Exempt). |
| February 2026 | **Pharmacy & Prescription Enhancements**: Added combination medicine content display in pharmacy module (shows combination details in blue italic text). Added "Send to Pharmacy" button in prescriptions module for each visit - creates pharmacy queue item with "Self Repeat by P/T" flag. Self-repeat prescriptions show orange badge in pharmacy queue and doctor panel past visits. Updated schemas to support source field in PharmacyQueueItem and isSelfRepeat/selfRepeatDate fields in DoctorVisit. |
| February 2026 | **Self-Repeat Fixes & Enhancements**: Fixed self-repeat prescriptions going to billing with "Self Repeat by P/T" fee type (0 amount, exempt status). Fixed deleted medicines appearing in prepared tab by filtering prescriptions with delete remark. Updated duplicate check to only prevent same-day sends (allows repeating prescriptions on different days). Started implementing prescription edit modal for send-to-pharmacy workflow. TODO: Complete edit modal, update visit history with edited flag, ensure self-repeat badge appears everywhere. |
